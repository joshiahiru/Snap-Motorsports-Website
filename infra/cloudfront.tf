# Origin Access Control so CloudFront can read the private S3 bucket.
...
# CloudFront distribution serving the static site over HTTPS.
resource "aws_cloudfront_distribution" "site" {
  enabled             = true
  is_ipv6_enabled     = true
  comment             = "${var.project_prefix} static site"
  ...
  # Route missing pages to the static 404 page.
  ...
  # Use default CloudFront cert until ACM validation completes, then switch to custom domain cert.
  viewer_certificate {
